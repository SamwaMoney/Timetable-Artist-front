        ...
        <M.Modal className={`${isMobile && "mobile-background"}`}>
            {isClicked ? (
                <div className={`red-background modal ${isMobile && "mobile-modal"}`}>
                    ...
                    <p className={`text ${isMobile && "mobile-text"}`}>삭제되었습니다.</p>
                    <button 
                        onClick={() => setIsDeleteModalOpen(false)} 
                        className={`red-btn big-btn ${isMobile && "mobile-big-btn"}`}
                    >
                        확인
                    </button>
                </div>
                ...
                <div className={`red-background modal ${isMobile && "mobile-modal"}`}>
                    <p className={`emoji ${isMobile && "mobile-emoji"}`}>(。・д・。)</p> {/* 원래 이모티콘이 깨져서 변경 */}
